...
- 颜色变体允许采购填中文，系统生成 SKU 时自动转码，例如 `黑色 -> BK`、`白色 -> WH`、`透明 -> CT`。
- 领星返回的重复 JSON 错误必须先归因再展示；例如 SKU 字符规则失败时，卡片只显示根因、异常 SKU、要改的字段和下一步，不逐条堆 `code/message/error_details/request_id`。
